#!/usr/bin/env python3
"""
Mirror the AI SDK documentation into a local markdown folder hierarchy.

High-level workflow:
  1. Pull `https://ai-sdk.dev/sitemap.xml` to discover every `/docs/` URL.
  2. Fetch each page, isolate the primary `<article data-docs-container>` node.
...
Usage (from repo root):
    uv run python src/DATA_SCRIPTS/mirror_ai_sdk_docs.py --output SPECIALIZED_CONTEXT_AND_DOCS/AI_SDK_DOCS_MIRROR

The default output path is `SPECIALIZED_CONTEXT_AND_DOCS/AI_SDK_DOCS_MIRROR`. Use `--clean`
...
DEFAULT_OUTPUT = REPO_ROOT / "SPECIALIZED_CONTEXT_AND_DOCS" / "AI_SDK_DOCS_MIRROR"
SITEMAP_URL = "https://ai-sdk.dev/sitemap.xml"
DOCS_PREFIX = "https://ai-sdk.dev/docs/"
USER_AGENT = "prompts-ai-sdk-docs-mirror/0.1 (+https://github.com/voxmenthe/prompts)"
FETCH_RETRIES = 3
...
def collect_jobs(limit: int | None) -> tuple[list[PageJob], dict[str, Path]]:
    ...
    urls = [
        ...
        if loc.text and loc.text.startswith(DOCS_PREFIX)
    ]
    ...
    for url in urls:
        ...
        raw_entries.append((url, docs_segments))
