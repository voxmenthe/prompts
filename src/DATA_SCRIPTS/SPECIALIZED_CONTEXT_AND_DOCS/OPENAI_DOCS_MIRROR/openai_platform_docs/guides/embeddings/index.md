...
```
query =
f"""Use the below article on the 2022 Winter Olympics to answer the subsequent question. If the answer cannot be found, write "I don't know."
...
search_reviews
(
df, product_description, n=
, pprint=
True
):


    embedding = get_embedding(product_description, model=
'text-embedding-3-small'
)
...
lambda
 x: cosine_similarity(x, embedding))
...
, n_lines=
    ...
    embedding = get_embedding(code_query, model=
'text-embedding-3-small'
)

    df[
'similarities'
] = df.code_embedding.apply(
lambda
 x: cosine_similarity(x, embedding))
...
# get embeddings for all strings
    ...
    embeddings = [embedding_from_string(string, model=model)
for
 string
in
 strings]
...
plt.scatter(x, y, c=color_indices, cmap=colormap, alpha=
0.3
)
...
label_embeddings = [get_embedding(label, model=model)
for
 label
in
 labels]
...
label_score
(
review_embedding, label_embeddings
):



return
 cosine_similarity(review_embedding, label_embeddings[
]) - cosine_similarity(review_embedding, label_embeddings[
])
...
```
user_embeddings = df.groupby(
'UserId'
).ada_embedding.apply(np.mean)

prod_embeddings = df.groupby(
'ProductId'
).ada_embedding.apply(np.mean)
```
