...
const
 location = sdpResponse.headers.get(
"Location"
);

const
 callId = location?.split(
"/"
).pop();
...
3. On a server, you can then [listen for events and configure the session](/docs/guides/realtime-conversations) just as you would from a typical Realtime API WebSocket connection, using that call ID with the URL
...
```python
POST https:
//my_website.com/webhook_endpoint


user-agent: OpenAI/
1.0
 (+https:
//platform.openai.com/docs/webhooks)
...
"id"
:
"evt_685343a1381c819085d44c354e1b330e"
,
...
"value"
:
"sip:+142555512112@sip.example.com"
 },
...
"value"
:
"sip:+18005551212@sip.example.com"
 },
...
"value"
:
"03782086-4ce9-44bf-8b0d-4e303d2cc590"
}
