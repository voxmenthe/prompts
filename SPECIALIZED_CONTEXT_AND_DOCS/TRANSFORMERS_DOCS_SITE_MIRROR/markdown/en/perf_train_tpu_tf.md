...
Create an instance of [tf.distribute.cluster\_resolver.TPUClusterResolver](https://www.tensorflow.org/api_docs/python/tf/distribute/cluster_resolver/TPUClusterResolver), and then connect to the remote cluster and initialize the TPUs.
...
Transformers preprocessing classes like [AutoImageProcessor](/docs/transformers/main/en/model_doc/auto#transformers.AutoImageProcessor) are framework-agnostic and can’t be compiled into a pipeline by `tf.data`. To get around this, get the normalization values (`mean` and `std`) from the [AutoImageProcessor](/docs/transformers/main/en/model_doc/auto#transformers.AutoImageProcessor) and use them in the `tf.data` pipeline.
...
### Stream with prepare\_tf\_dataset

[prepare\_tf\_dataset()](/docs/transformers/main/en/main_classes/model#transformers.TFPreTrainedModel.prepare_tf_dataset) creates a `tf.data` pipeline that loads samples from [tf.data.Dataset](https://www.tensorflow.org/api_docs/python/tf/data/Dataset). The pipeline uses tf.numpy\_function or [from\_generator](https://huggingface.co/docs/datasets/main/en/package_reference/main_classes#datasets.Dataset.from_generator), which can’t be compiled by TensorFlow, to access the underlying `tf.data.Dataset`. It also won’t work on a Colab TPU or TPU Nodes because the pipeline streams data from a local disk. Refer to the table below to help you decide whether this approach is helpful for you.
...
[prepare\_tf\_dataset()](/docs/transformers/main/en/main_classes/model#transformers.TFPreTrainedModel.prepare_tf_dataset) only works on [TPU VM](#tpu-types). Add the tokenizer output as columns in the dataset since the dataset is stored on disk, which means it can handle data larger than the available memory. Use [prepare\_tf\_dataset()](/docs/transformers/main/en/main_classes/model#transformers.TFPreTrainedModel.prepare_tf_dataset) to stream data from the dataset by wrapping it with a `tf.data` pipeline.
...
[< > Update on GitHub](https://github.com/huggingface/transformers/blob/main/docs/source/en/perf_train_tpu_tf.md)
