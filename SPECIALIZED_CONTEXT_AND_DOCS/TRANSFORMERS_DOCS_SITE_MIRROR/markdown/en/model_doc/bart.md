...
- BART doesn't use `token_type_ids` for sequence classification. Use [BartTokenizer](/docs/transformers/main/en/model_doc/longformer#transformers.RobertaTokenizer) or [encode()](/docs/transformers/main/en/internal/tokenization_utils#transformers.PreTrainedTokenizerBase.encode) to get the proper splitting.
- The forward pass of [BartModel](/docs/transformers/main/en/model_doc/bart#transformers.BartModel) creates the `decoder_input_ids` if they're not passed. This can be different from other model APIs, but it is a useful feature for mask-filling tasks.
...
- [generate()](/docs/transformers/main/en/main_classes/text_generation#transformers.GenerationMixin.generate) should be used for conditional generation tasks like summarization.

## BartConfig[[transformers.BartConfig]]

#### transformers.BartConfig[[transformers.BartConfig]]

[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/bart/configuration_bart.py#L24)

This is the configuration class to store the configuration of a [BartModel](/docs/transformers/main/en/model_doc/bart#transformers.BartModel). It is used to instantiate a BART
...
vocab_size (`int`, *optional*, defaults to 50265) : Vocabulary size of the BART model. Defines the number of different tokens that can be represented by the `inputs_ids` passed when calling [BartModel](/docs/transformers/main/en/model_doc/bart#transformers.BartModel).
...
num_labels (`int`, *optional*, defaults to 3) : The number of labels to use in [BartForSequenceClassification](/docs/transformers/main/en/model_doc/bart#transformers.BartForSequenceClassification).
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/bart/modeling_bart.py#L883)
...
elements depending on the configuration ([BartConfig](/docs/transformers/main/en/model_doc/bart#transformers.BartConfig)) and inputs.
...
- **encoder_attentions** (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`) -- Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
  ...
  self-attention heads.
The [BartModel](/docs/transformers/main/en/model_doc/bart#transformers.BartModel) forward method, overrides the `__call__` special method.
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/bart/modeling_bart.py#L1024)
...
- **encoder_attentions** (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`) -- Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
  ...
  Attentions weights of the encoder, after the attention softmax, used to compute the weighted average in the
  self-attention heads.
The [BartForConditionalGeneration](/docs/transformers/main/en/model_doc/bart#transformers.BartForConditionalGeneration) forward method, overrides the `__call__` special method.
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/bart/modeling_bart.py#L1205)
...
- **encoder_attentions** (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`) -- Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
  ...
  Attentions weights of the encoder, after the attention softmax, used to compute the weighted average in the
  self-attention heads.
The [BartForSequenceClassification](/docs/transformers/main/en/model_doc/bart#transformers.BartForSequenceClassification) forward method, overrides the `__call__` special method.
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/bart/modeling_bart.py#L1338)
...
- **encoder_attentions** (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`) -- Tuple of `torch.FloatTensor` (one for each layer) of shape `(batch_size, num_heads, sequence_length,
  ...
  Attentions weights of the encoder, after the attention softmax, used to compute the weighted average in the
  self-attention heads.
The [BartForQuestionAnswering](/docs/transformers/main/en/model_doc/bart#transformers.BartForQuestionAnswering) forward method, overrides the `__call__` special method.
...
[Source](https://github.com/huggingface/transformers/blob/main/src/transformers/models/bart/modeling_bart.py#L1476)
...
The [BartForCausalLM](/docs/transformers/main/en/model_doc/bart#transformers.BartForCausalLM) forward method, overrides the `__call__` special method.
