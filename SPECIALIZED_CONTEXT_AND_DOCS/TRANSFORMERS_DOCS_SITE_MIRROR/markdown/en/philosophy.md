...
- On top of those three base classes, the library provides two APIs: [pipeline()](/docs/transformers/main/en/main_classes/pipelines#transformers.pipeline) for quickly
    using a model for inference on a given task and [Trainer](/docs/transformers/main/en/main_classes/trainer#transformers.Trainer) to quickly train or fine-tune a PyTorch model.
