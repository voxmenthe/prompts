...
As explained in [Transformers Agents](transformers_agents) agents can run in [run()](/docs/transformers/main/en/main_classes/agent#transformers.Agent.run) and [chat()](/docs/transformers/main/en/main_classes/agent#transformers.Agent.chat) mode.
...
We’ll start by loading the custom tools with the convenient [load\_tool()](/docs/transformers/main/en/main_classes/agent#transformers.load_tool) function:
...
[< > Update on GitHub](https://github.com/huggingface/transformers/blob/main/docs/source/en/custom_tools.md)
