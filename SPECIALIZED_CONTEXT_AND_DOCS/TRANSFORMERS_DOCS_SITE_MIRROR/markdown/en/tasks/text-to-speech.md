...
...     return 100 >> dataset = dataset.filter(select_speaker, input_columns=["speaker_id"])
```
...
...     return input_length >> dataset = dataset.filter(is_not_too_long, input_columns=["input_ids"])
>>> len(dataset)
8259
```
