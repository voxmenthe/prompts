...
[Trainer](./trainer) supports many useful training features that can be configured through [TrainingArguments](/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments). This section highlights some of the more important features for optimizing training.
...
Configure `per_device_train_batch_size()` in [TrainingArguments](/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments).
...
Configure `per_device_train_batch_size()` in [TrainingArguments](/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments) to enable gradient accumulation.
...
Configure `gradient_checkpointing()` in [TrainingArguments](/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments) to enable gradient checkpointing.
...
Configure `fp16()` in [TrainingArguments](/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments) to enable mixed precision training with the fp16 data type.
...
Configure `bf16()` in [TrainingArguments](/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments) to enable mixed precision training with the bf16 data type.
...
Configure [tf32()](https://huggingface.co/docs/transformers/main_classes/trainer#transformers.TrainingArguments.tf32) in [TrainingArguments](/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments) to enable mixed precision training with tf32 mode.
...
Configure `optim()` in [TrainingArguments](/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments) to choose an optimizer.
...
Configure `dataloader_pin_memory()` and `dataloader_num_workers()` in [TrainingArguments](/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments) to allocate pinned memory and increase the number of workers.
...
Use [torch_empty_cache_steps()](https://huggingface.co/docs/transformers/main_classes/trainer#transformers.TrainingArguments.torch_empty_cache_steps) in [TrainingArguments](/docs/transformers/main/en/main_classes/trainer#transformers.TrainingArguments) to enable it after a certain number of training steps.
...
SDPA is enabled by default for PyTorch 2.1.1+, but it can be explicitly enabled by setting `attn_implementation="sdpa"` in [from_pretrained()](/docs/transformers/main/en/main_classes/model#transformers.PreTrainedModel.from_pretrained).
