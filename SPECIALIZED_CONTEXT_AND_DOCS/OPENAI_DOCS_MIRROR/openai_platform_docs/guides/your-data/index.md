...
*   Audio outputs application state is stored for 1 hour to enable [multi-turn conversations](http://platform.openai.com/docs/guides/your-data?timeout=30).
...
*   Image generation is Zero Data Retention compatible when using `gpt-image-1` and `gpt-image-1-mini`, not when using `dall-e-3` or `dall-e-2`.
...
*   The `v1/video` is not compatible with data retention controls. If your organization has data retention controls enabled, configure a project with its retention setting set to **None** as described in [Configuring data retention controls](https://platform.openai.com/docs/guides/your-data#configuring-data-retention-controls) to use `/v1/video` with that project.
...
| /v1/images/edits | gpt-image-1 gpt-image-1-mini | All |
| /v1/images/generations | dall-e-3 gpt-image-1 gpt-image-1-mini | All |
| /v1/moderations | text-moderation-latest* omni-moderation-latest | All |
| /v1/realtime | gpt-4o-realtime-preview-2025-06-03 gpt-realtime gpt-realtime-mini | US and EU |
