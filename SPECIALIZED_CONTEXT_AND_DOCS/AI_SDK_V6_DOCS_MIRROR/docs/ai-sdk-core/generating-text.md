...
Advanced LLM features such as [tool calling](tools-and-tool-calling.md) and [structured data generation](generating-structured-data.md) are built on top of text generation.

## `generateText`

You can generate text using the [`generateText`](../reference/ai-sdk-core/generate-text.md) function. This function is ideal for non-interactive use cases where you need to write text (e.g. drafting email or summarizing web pages) and for agents that use tools.
...
You can use more [advanced prompts](prompts.md) to generate text with more complex instructions and content:
...
[API Reference](../reference/ai-sdk-core/generate-text.md#on-finish)
).
...
AI SDK Core provides the [`streamText`](../reference/ai-sdk-core/stream-text.md) function which simplifies streaming text from LLMs:
...
The result object contains several helper functions to make the integration into [AI SDK UI](../ai-sdk-ui-folder-description.md) easier:
...
[API Reference](../reference/ai-sdk-core/stream-text.md#on-finish)
).
It contains the text, usage information, finish reason, messages, steps, total usage, and more:
...
The AI SDK Core provides a [`smoothStream` function](../reference/ai-sdk-core/smooth-stream.md) that
