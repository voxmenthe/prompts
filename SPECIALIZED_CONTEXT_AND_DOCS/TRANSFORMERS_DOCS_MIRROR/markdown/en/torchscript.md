...
2. enabling the `torchscript` parameter in either `~PretrainedConfig.torchscript` for a randomly initialized model or [from\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) for a pretrained model
...
[< > Update on GitHub](https://github.com/huggingface/transformers/blob/main/docs/source/en/torchscript.md)
