...
You should also use the `@auto_docstring` decorator for classes that inherit from [ModelOutput](/docs/transformers/v4.56.2/en/main_classes/output#transformers.utils.ModelOutput).
...
5. Using a templating system to allow predefined docstrings to include dynamic information from Transformers’ [auto\_modules](https://github.com/huggingface/transformers/tree/main/src/transformers/models/auto) such as `{{processor_class}}` and `{{config_class}}`.
...
7. Adding return values to the docstring. For methods like `forward`, the decorator automatically generates the `Returns` field in the docstring based on the method’s return type annotation.
   ...
   For example, if a method returns a [ModelOutput](/docs/transformers/v4.56.2/en/main_classes/output#transformers.utils.ModelOutput) subclass, `@auto_docstring` extracts the field descriptions from the class’ docstring to create a comprehensive return value description. You can also manually specify a custom `Returns` field in a functions docstring.
...
* Use `@auto_docstring` for new PyTorch model classes ([PreTrainedModel](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel) subclasses) and their primary methods like `forward` or `get_text_features`.
...
[< > Update on GitHub](https://github.com/huggingface/transformers/blob/main/docs/source/en/auto_docstring.md)
