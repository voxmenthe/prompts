...
of the model we use in this guide. Instead, we can use the [LayoutLMv2Processor](/docs/transformers/v4.56.2/en/model_doc/layoutlmv2#transformers.LayoutLMv2Processor) on the original data for both OCR and
...
are preprocessed according to the model’s expectations. Let’s start by loading the [LayoutLMv2Processor](/docs/transformers/v4.56.2/en/model_doc/layoutlmv2#transformers.LayoutLMv2Processor), which internally combines an image processor that can handle image data and a tokenizer that can encode text data.
...
To apply this preprocessing to the entire dataset in a fast way, use [map](https://huggingface.co/docs/datasets/v4.1.0/en/package_reference/main_classes#datasets.Dataset.map).
...
if the current word in the `words_list` (words\_list[i]) is equal to the first word of answer\_list (answer\_list[0]) and if
...
and its ending index (idx + len(answer\_list) - 1). If more than one match was found, the function will return only the first one.
...
of your time, this guide skips the evaluation step. The [Trainer](/docs/transformers/v4.56.2/en/main_classes/trainer#transformers.Trainer) still calculates the evaluation loss during training so
...
* Load the model with [AutoModelForDocumentQuestionAnswering](/docs/transformers/v4.56.2/en/model_doc/auto#transformers.AutoModelForDocumentQuestionAnswering) using the same checkpoint as in the preprocessing.
* Define your training hyperparameters in [TrainingArguments](/docs/transformers/v4.56.2/en/main_classes/trainer#transformers.TrainingArguments).
* Define a function to batch examples together, here the [DefaultDataCollator](/docs/transformers/v4.56.2/en/main_classes/data_collator#transformers.DefaultDataCollator) will do just fine
* Pass the training arguments to [Trainer](/docs/transformers/v4.56.2/en/main_classes/trainer#transformers.Trainer) along with the model, dataset, and data collator.
* Call [train()](/docs/transformers/v4.56.2/en/main_classes/trainer#transformers.Trainer.train) to finetune your model.
...
In the [TrainingArguments](/docs/transformers/v4.56.2/en/main_classes/trainer#transformers.TrainingArguments) use `output_dir` to specify where to save your model, and configure hyperparameters as you see fit.
...
Finally, bring everything together, and call [train()](/docs/transformers/v4.56.2/en/main_classes/trainer#transformers.Trainer.train):
...
way to try out your finetuned model for inference is to use it in a [Pipeline](/docs/transformers/v4.56.2/en/main_classes/pipelines#transformers.Pipeline).
...
3. The model returns `start_logits` and `end_logits`, which indicate which token is at the start of the answer and
   which token is at the end of the answer. Both have shape (batch\_size, sequence\_length).
...
[< > Update on GitHub](https://github.com/huggingface/transformers/blob/main/docs/source/en/tasks/document_question_answering.md)
