...
* Use the [ORTModel](https://huggingface.co/docs/optimum/v1.27.0/en/onnxruntime/package_reference/modeling_ort#optimum.onnxruntime.ORTModel) API to run a model with [ONNX Runtime](https://onnxruntime.ai/).
...
The `optimum.onnxruntime` module supports programmatically exporting a Transformers model. Instantiate a [ORTModel](https://huggingface.co/docs/optimum/v1.27.0/en/onnxruntime/package_reference/modeling_ort#optimum.onnxruntime.ORTModel) for a task and set `export=True`. Use `~OptimizedModel.save_pretrained` to save the ONNX model.
...
[< > Update on GitHub](https://github.com/huggingface/transformers/blob/main/docs/source/en/serialization.md)
