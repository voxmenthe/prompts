...
For text generation with 8-bit quantization, you should use [generate()](/docs/transformers/v4.56.2/en/main_classes/text_generation#transformers.GenerationMixin.generate) instead of the high-level [Pipeline](/docs/transformers/v4.56.2/en/main_classes/pipelines#transformers.Pipeline) API. The [Pipeline](/docs/transformers/v4.56.2/en/main_classes/pipelines#transformers.Pipeline) returns slower performance because it isn’t optimized for 8-bit models, and some sampling strategies (nucleus sampling) also aren’t supported.

Set up a [BitsAndBytesConfig](/docs/transformers/v4.56.2/en/main_classes/quantization#transformers.BitsAndBytesConfig) and set `load_in_8bit=True` to load a model in 8-bit precision. The [BitsAndBytesConfig](/docs/transformers/v4.56.2/en/main_classes/quantization#transformers.BitsAndBytesConfig) is passed to the `quantization_config` parameter in [from\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.from_pretrained).
...
Optimum provides the [ORTModel](https://huggingface.co/docs/optimum/v1.27.0/en/onnxruntime/package_reference/modeling_ort#optimum.onnxruntime.ORTModel) class for loading ONNX models. Set the `provider` parameter according to the table below.
...
Now you can use the model for inference in a [Pipeline](/docs/transformers/v4.56.2/en/main_classes/pipelines#transformers.Pipeline).
...
Some BetterTransformer features are being upstreamed to Transformers with default support for native [torch.nn.functional.scaled\_dot\_product\_attention](https://pytorch.org/docs/stable/generated/torch.nn.functional.scaled_dot_product_attention.html) (SDPA). BetterTransformer has a wider coverage than the Transformers SDPA integration, but you can expect more and more architectures to natively support SDPA in Transformers.
...
Call [reverse\_bettertransformer()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.reverse_bettertransformer) and save it first to return the model to the original Transformers model.
