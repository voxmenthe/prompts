...
type was used by the pretrained model. For instance, if we look at [BertTokenizer](/docs/transformers/v4.56.2/en/model_doc/bert#transformers.BertTokenizer), we can see
...
seen before, by decomposing them into known subwords. For instance, the [BertTokenizer](/docs/transformers/v4.56.2/en/model_doc/bert#transformers.BertTokenizer) tokenizes
...
As another example, [XLNetTokenizer](/docs/transformers/v4.56.2/en/model_doc/xlnet#transformers.XLNetTokenizer) tokenizes our previously exemplary text as follows:
...
the wordsx1,…,xNx\_{1}, \dots, x\_{N}x1​,…,xN​ and that the set of all possible tokenizations for a wordxix\_{i}xi​ is
defined asS(xi)S(x\_{i})S(xi​), then the overall loss is defined as
L=−∑i=1Nlog⁡(∑x∈S(xi)p(x))\mathcal{L} = -\sum\_{i=1}^{N} \log \left ( \sum\_{x \in S(x\_{i})} p(x) \right )L=−i=1∑N​log​x∈S(xi​)∑​p(x)​
...
The [XLNetTokenizer](/docs/transformers/v4.56.2/en/model_doc/xlnet#transformers.XLNetTokenizer) uses SentencePiece for example, which is also why in the example earlier the
