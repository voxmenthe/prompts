...
Transformers provides many pretrained models that are ready to use with a single line of code. It requires a model class and the [from\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) method.

Call [from\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) to download and load a model’s weights and configuration stored on the Hugging Face [Hub](https://hf.co/models).

The [from\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) method loads weights stored in the [safetensors](https://hf.co/docs/safetensors/index) file format if they’re available. Traditionally, PyTorch model weights are serialized with the [pickle](https://docs.python.org/3/library/pickle.html) utility which is known to be unsecure. Safetensor files are more secure and faster to load.
...
1. A barebones model, like [AutoModel](/docs/transformers/v4.56.2/en/model_doc/auto#transformers.AutoModel) or [LlamaModel](/docs/transformers/v4.56.2/en/model_doc/llama#transformers.LlamaModel), that outputs hidden states.
2. A model with a specific *head* attached, like [AutoModelForCausalLM](/docs/transformers/v4.56.2/en/model_doc/auto#transformers.AutoModelForCausalLM) or [LlamaForCausalLM](/docs/transformers/v4.56.2/en/model_doc/llama#transformers.LlamaForCausalLM), for performing specific tasks.

## Model classes

To get a pretrained model, you need to load the weights into the model. This is done by calling [from\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) which accepts weights from the Hugging Face Hub or a local directory.
...
The [save\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.save_pretrained) method automatically shards checkpoints larger than 10GB.
...
For example, create some shards checkpoints for [BioMistral/BioMistral-7B](https://hf.co/BioMistral/BioMistral-7B) in [save\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.save_pretrained).
...
Reload the sharded checkpoint with [from\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.from_pretrained).
...
Sharded checkpoints can also be directly loaded with [load\_sharded\_checkpoint()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.modeling_utils.load_sharded_checkpoint).
...
The [save\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.save_pretrained) method creates an index file that maps parameter names to the files they’re stored in. The index file has two keys, `metadata` and `weight_map`.
...
[from\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) is supercharged with Accelerate’s [Big Model Inference](https://hf.co/docs/accelerate/usage_guides/big_modeling) feature.
...
Set [device\_map](https://github.com/huggingface/transformers/blob/026a173a64372e9602a16523b8fae9de4b0ff428/src/transformers/modeling_utils.py#L3061) to `"auto"` to enable Big Model Inference.
...
The `dtype` parameter can also be configured in [AutoConfig](/docs/transformers/v4.56.2/en/model_doc/auto#transformers.AutoConfig) for models instantiated from scratch.
...
Custom models builds on Transformers’ configuration and modeling classes, supports the [AutoClass](#autoclass) API, and are loaded with [from\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.from_pretrained). The difference is that the modeling code is *not* from Transformers.
...
Set `trust_remote_code=True` in [from\_pretrained()](/docs/transformers/v4.56.2/en/main_classes/model#transformers.PreTrainedModel.from_pretrained) to load a custom model.
...
[< > Update on GitHub](https://github.com/huggingface/transformers/blob/main/docs/source/en/models.md)
