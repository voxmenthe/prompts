...
2. If the target tool _changes _the format in some way, uv is then locked-in to changing it in equivalent ways. 

3. If that configuration is versioned in some way, uv would need to know _which version _of the target tool the user is expecting to use. 
...
5. It can lead to user confusion, since uv would be reading settings that don't actually affect its behavior, and many users may _not _expect uv to read configuration files intended for other tools. 
...
uv also supports pinning packages to dedicated indexes (see: [_Indexes _](../../concepts/indexes/#pinning-a-package-to-an-index)), such that a given package is _always _installed from a specific index. 
...
```
[#__codelineno-0-1](#__codelineno-0-1)uv pip install wheel && uv pip install --no-build-isolation biopython==1.77
...
While uv includes first-class support for URL dependencies (e.g., `ruff @ https://... `), it differs from pip in its handling of _transitive _URL dependencies in two ways. 

First, uv makes the assumption that non-URL dependencies do not introduce URL dependencies into the resolution. In other words, it assumes that dependencies fetched from a registry do not themselves depend on URLs. If a non-URL dependency _does _introduce a URL dependency, uv will reject the URL dependency during resolution. (Note that PyPI does not allow published packages to depend on URL dependencies; other registries may be more permissive.) 

Second, if a constraint ( `--constraint `) or override ( `--override `) is defined using a direct URL dependency, and the constrained package has a direct URL dependency of its own, uv _may _reject that transitive direct URL dependency during resolution, if the URL isn't referenced elsewhere in the set of input requirements. 
...
```
[#__codelineno-1-1](#__codelineno-1-1)starlette
[#__codelineno-1-2](#__codelineno-1-2)fastapi
...
```
[#__codelineno-2-1](#__codelineno-2-1)# This file was autogenerated by uv via the following command:
[#__codelineno-2-2](#__codelineno-2-2)#    uv pip compile requirements.in
[#__codelineno-2-3](#__codelineno-2-3)annotated-types==0.6.0
[#__codelineno-2-4](#__codelineno-2-4)    # via pydantic
[#__codelineno-2-5](#__codelineno-2-5)anyio==4.3.0
[#__codelineno-2-6](#__codelineno-2-6)    # via starlette
[#__codelineno-2-7](#__codelineno-2-7)fastapi==0.1.17
[#__codelineno-2-8](#__codelineno-2-8)idna==3.6
[#__codelineno-2-9](#__codelineno-2-9)    # via anyio
[#__codelineno-2-10](#__codelineno-2-10)pydantic==2.6.3
[#__codelineno-2-11](#__codelineno-2-11)    # via fastapi
[#__codelineno-2-12](#__codelineno-2-12)pydantic-core==2.16.3
[#__codelineno-2-13](#__codelineno-2-13)    # via pydantic
[#__codelineno-2-14](#__codelineno-2-14)sniffio==1.3.1
[#__codelineno-2-15](#__codelineno-2-15)    # via anyio
[#__codelineno-2-16](#__codelineno-2-16)starlette==0.37.2
[#__codelineno-2-17](#__codelineno-2-17)    # via fastapi
[#__codelineno-2-18](#__codelineno-2-18)typing-extensions==4.10.0
[#__codelineno-2-19](#__codelineno-2-19)    # via
[#__codelineno-2-20](#__codelineno-2-20)    #   pydantic
[#__codelineno-2-21](#__codelineno-2-21)    #   pydantic-core
...
```
[#__codelineno-3-1](#__codelineno-3-1)# This file was autogenerated by uv via the following command:
[#__codelineno-3-2](#__codelineno-3-2)#    uv pip compile requirements.in
[#__codelineno-3-3](#__codelineno-3-3)annotated-types==0.6.0
[#__codelineno-3-4](#__codelineno-3-4)    # via pydantic
[#__codelineno-3-5](#__codelineno-3-5)anyio==4.3.0
[#__codelineno-3-6](#__codelineno-3-6)    # via starlette
[#__codelineno-3-7](#__codelineno-3-7)fastapi==0.110.0
[#__codelineno-3-8](#__codelineno-3-8)idna==3.6
[#__codelineno-3-9](#__codelineno-3-9)    # via anyio
[#__codelineno-3-10](#__codelineno-3-10)pydantic==2.6.3
[#__codelineno-3-11](#__codelineno-3-11)    # via fastapi
[#__codelineno-3-12](#__codelineno-3-12)pydantic-core==2.16.3
[#__codelineno-3-13](#__codelineno-3-13)    # via pydantic
[#__codelineno-3-14](#__codelineno-3-14)sniffio==1.3.1
[#__codelineno-3-15](#__codelineno-3-15)    # via anyio
[#__codelineno-3-16](#__codelineno-3-16)starlette==0.36.3
[#__codelineno-3-17](#__codelineno-3-17)    # via fastapi
[#__codelineno-3-18](#__codelineno-3-18)typing-extensions==4.10.0
[#__codelineno-3-19](#__codelineno-3-19)    # via
[#__codelineno-3-20](#__codelineno-3-20)    #   fastapi
[#__codelineno-3-21](#__codelineno-3-21)    #   pydantic
[#__codelineno-3-22](#__codelineno-3-22)    #   pydantic-core
...
In some cases, `uv pip check `will surface diagnostics that `pip check `does not, and vice versa. For example, unlike `uv pip check `, `pip check `will _not _warn when multiple versions of a package are installed in the current environment. 
...
However, when a dependency is provided as a direct URL (e.g., `uv pip install https://... `), pip does _not _enforce `--only-binary `, and will build source distributions for all such packages. 

uv, meanwhile, _does _enforce `--only-binary `for direct URL dependencies, with one exception: given `uv pip install https://... --only-binary flask `, uv _will _build the source distribution at the given URL if it cannot infer the package name ahead of time, since uv can't determine whether the package is "allowed" in such cases without building its metadata. 
...
The `--no-binary `argument is used to restrict installation to source distributions. When `--no-binary `is provided, uv will refuse to install pre-built binary distributions, but _will _reuse any binary distributions that are already present in the local cache. 
...
```
[#__codelineno-4-1](#__codelineno-4-1)from __future__ import annotations
[#__codelineno-4-2](#__codelineno-4-2)manylinux1_compatible = False
[#__codelineno-4-3](#__codelineno-4-3)manylinux2010_compatible = False
[#__codelineno-4-4](#__codelineno-4-4)manylinux2014_compatible = False
[#__codelineno-4-5](#__codelineno-4-5)
[#__codelineno-4-6](#__codelineno-4-6)
[#__codelineno-4-7](#__codelineno-4-7)def manylinux_compatible(*_, **__):  # PEP 600
[#__codelineno-4-8](#__codelineno-4-8)    return False
...
When constraints are provided via `--constraint `(or `UV_CONSTRAINT `), uv will _not _apply the constraints when resolving build dependencies (i.e., to build a source distribution). Instead, build constraints should be provided via the dedicated `--build-constraint `(or `UV_BUILD_CONSTRAINT `) setting. 
...
While this is not strictly compliant with [PEP 440](https://peps.python.org/pep-0440/), it _is _consistent with [pip](https://github.com/pypa/pip/blob/24.1.1/src/pip/_internal/resolution/resolvelib/candidates.py#L540). 
...
```
[#__codelineno-5-1](#__codelineno-5-1)(venv) $ diff --side-by-side  <(pip list) <(uv pip list)
[#__codelineno-5-2](#__codelineno-5-2)Package          Version                    Package          Version
[#__codelineno-5-3](#__codelineno-5-3)---------------- -------                    ---------------- -------
[#__codelineno-5-4](#__codelineno-5-4)docstring_parser 0.16                         | docstring-parser 0.16
[#__codelineno-5-5](#__codelineno-5-5)jaraco.classes   3.4.0                        | jaraco-classes   3.4.0
[#__codelineno-5-6](#__codelineno-5-6)more-itertools   10.7.0                         more-itertools   10.7.0
[#__codelineno-5-7](#__codelineno-5-7)pip              25.1                           pip              25.1
[#__codelineno-5-8](#__codelineno-5-8)PyMuPDFb         1.24.10                      | pymupdfb         1.24.10
[#__codelineno-5-9](#__codelineno-5-9)PyPDF2           3.0.1                        | pypdf2           3.0.1
