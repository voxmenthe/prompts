...
```
[#__codelineno-0-1](#__codelineno-0-1)$ uv run --with jupyter jupyter lab
...
```
[#__codelineno-1-1](#__codelineno-1-1)$ uv add --dev ipykernel
...
```
[#__codelineno-2-1](#__codelineno-2-1)$ uv run ipython kernel install --user --env VIRTUAL_ENV $(pwd)/.venv --name=project
...
```
[#__codelineno-3-1](#__codelineno-3-1)$ uv run --with jupyter jupyter lab
...
Though `uv run --with jupyter `runs in an isolated environment, within the notebook itself, `!uv add `and related commands will modify the _project's _environment, even without a kernel. 
...
However, since the Jupyter server is the "active" environment, `!uv pip install `will install package's into _Jupyter's _environment, not the project environment. Such dependencies will persist for the lifetime of the Jupyter server, but may disappear on subsequent `jupyter `invocations. 
...
```
[#__codelineno-4-1](#__codelineno-4-1)$ uv venv --seed
[#__codelineno-4-2](#__codelineno-4-2)$ uv run --with jupyter jupyter lab

```

Subsequent `%pip install `invocations within the notebook will install packages into the project's virtual environment. However, such modifications will _not _be reflected in the project's `pyproject.toml `or `uv.lock `files. 
...
```
[#__codelineno-5-1](#__codelineno-5-1)$ uv venv --seed
[#__codelineno-5-2](#__codelineno-5-2)$ uv pip install pydantic
[#__codelineno-5-3](#__codelineno-5-3)$ uv pip install jupyterlab
[#__codelineno-5-4](#__codelineno-5-4)$ .venv/bin/jupyter lab
...
```
[#__codelineno-6-1](#__codelineno-6-1)PS> uv venv --seed
[#__codelineno-6-2](#__codelineno-6-2)PS> uv pip install pydantic
[#__codelineno-6-3](#__codelineno-6-3)PS> uv pip install jupyterlab
[#__codelineno-6-4](#__codelineno-6-4)PS> .venv\Scripts\jupyter lab
...
```
[#__codelineno-7-1](#__codelineno-7-1)# Create a project.
[#__codelineno-7-2](#__codelineno-7-2)$ uv init project
[#__codelineno-7-3](#__codelineno-7-3)
[#__codelineno-7-4](#__codelineno-7-4)# Move into the project directory.
[#__codelineno-7-5](#__codelineno-7-5)$ cd project
[#__codelineno-7-6](#__codelineno-7-6)
[#__codelineno-7-7](#__codelineno-7-7)# Add ipykernel as a dev dependency.
[#__codelineno-7-8](#__codelineno-7-8)$ uv add --dev ipykernel
[#__codelineno-7-9](#__codelineno-7-9)
[#__codelineno-7-10](#__codelineno-7-10)# Open the project in VS Code.
[#__codelineno-7-11](#__codelineno-7-11)$ code .
...
```
[#__codelineno-8-1](#__codelineno-8-1)$ uv add --dev uv
