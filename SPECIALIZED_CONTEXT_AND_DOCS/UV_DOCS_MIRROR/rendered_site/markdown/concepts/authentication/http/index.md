...
## [netrc files](#netrc-files)

[`.netrc `](https://everything.curl.dev/usingcurl/netrc)files are a long-standing plain text format for storing credentials on a system. 
...
## [The uv credentials store](#the-uv-credentials-store)

uv can read and write credentials from a store using the [`uv auth `commands](../cli/). 
...
!!! note "Note"
    ...
    A secure, system native storage mechanism is in [preview](../../preview/)— it is still experimental and being actively developed. In the future, this will become the default storage mechanism. 
...
## [Keyring providers](#keyring-providers)

A keyring provider is a concept from `pip `allowing retrieval of credentials from an interface matching the popular [keyring](https://github.com/jaraco/keyring)Python package. 
...
## [Persistence of credentials](#persistence-of-credentials)
...
When using `uv add `, uv _will not _persist index credentials to the `pyproject.toml `or `uv.lock `. These files are often included in source control and distributions, so it is generally unsafe to include credentials in them. However, uv _will _persist credentials for direct URLs, i.e., `package @ https://username:password:example.com/foo.whl `, as there is not currently a way to otherwise provide those credentials. 
