...
- `git+ssh://git@ /... `(e.g., `git+ssh:// [[email protected]](/cdn-cgi/l/email-protection)/astral-sh/uv `) 
...
- `git+https:// : @ /... `(e.g., `git+https://git: [[email protected]](/cdn-cgi/l/email-protection)/astral-sh/uv `) 

- `git+https:// @ /... `(e.g., `git+https:// [[email protected]](/cdn-cgi/l/email-protection)/astral-sh/uv `) 
...
When using `uv add `, uv _will not _persist Git credentials to the `pyproject.toml `or `uv.lock `. These files are often included in source control and distributions, so it is generally unsafe to include credentials in them. 
...
You _may _force uv to persist Git credentials by passing the `--raw `option to `uv add `. However, we strongly recommend setting up a [credential helper](#git-credential-helpers)instead. 
...
```
[#__codelineno-0-1](#__codelineno-0-1)$ gh auth login
